...
@testset "Deep copy on $exec_space_type in $(dim)D with $type" for 
    ...
    @testset "View{$type, $dim, $src_layout, $src_space} => View{$type, $dim, $dst_layout, $dst_space}" for
        ...
        # TODO: handle the edge case where layouts are non-continuous on (possibly) inaccessible
        #  memory spaces (see https://kokkos.github.io/kokkos-core-wiki/API/core/view/deep_copy.html#requirements)
        # if (src_layout != dst_layout || src_layout == Kokkos.LayoutStride)
        #         && (exec_space_type === :no_exec_space || (accessible(exec_space_type, src_space) && accessible(exec_space_type, dst_space)))
